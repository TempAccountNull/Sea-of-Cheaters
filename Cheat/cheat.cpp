                    ...
                    if (cfg.visuals.islands.bName)
                    {
                        do {
                            ...
                            for (auto i = 0u; i < islandDataEntries.Count; i++)
                            {
                                ...
                                auto const CaptureParams = WorldMapData->CaptureParams;
                                if (!CaptureParams) continue;

                                const FVector islandLoc = CaptureParams->WorldSpaceCameraPosition;
        ...
        if (ImGui::BeginTabBar("Bars")) {
            ...
            if (ImGui::BeginTabItem("Misc")) {
                ...
                if (ImGui::BeginChild("SwordMods", ImVec2(0.f, 310.f), true, 0 | ImGuiWindowFlags_NoScrollWithMouse))
                {
                    ...
                    ImGui::Checkbox("Heavy NoClamp", &cfg.misc.sword.noclamp);
                    ImGui::Checkbox("Sword NoReduce", &cfg.misc.sword.noblockreduce);
                }
                ImGui::EndChild();
